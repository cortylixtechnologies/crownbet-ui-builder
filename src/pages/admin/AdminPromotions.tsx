...
const AdminPromotions = () => {
  const { store, addPromo, updatePromo, deletePromo } = useAdmin();
  const [form, setForm] = useState({ title: "", emoji: "🎁", color: colorPresets[0], to: "/promotions" });

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.title) return toast.error("Title required");
    addPromo({ ...form, active: true });
    setForm({ title: "", emoji: "🎁", color: colorPresets[0], to: "/promotions" });
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={submit} className="grid md:grid-cols-4 gap-3">
            ...
            <Input placeholder="Link (/promotions)" value={form.to} onChange={(e) => setForm({ ...form, to: e.target.value })} />
