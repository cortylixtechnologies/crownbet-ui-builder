...
const AdminMatches = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>All Matches ({matches.length})</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {matches.map((m) => (
            <div key={m.id} className="grid grid-cols-[1fr_auto] gap-3 items-center bg-secondary rounded-lg p-3">
              <div>
                <div className="text-xs text-muted-foreground">{m.league} · {m.match_date} {m.match_time}</div>
